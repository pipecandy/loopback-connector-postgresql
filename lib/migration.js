...
function mixinMigration(PostgreSQL) {
  ...
  PostgreSQL.prototype.getTableStatus = function(model, cb) {
    ...
    this.execute('SELECT column_name AS "column", data_type AS "type", ' +
      ...
      this.schema(model) + '\'', decoratedTableDataCallback);
  ...
  PostgreSQL.prototype.autoupdate = function(models, cb) {
    ...
    async.eachSeries(models, function(model, done) {
      const schemaName = self.escapeName(self.schema(model));
      if(!schemaName) return done(null);
      ...
      if(asyncErr) return cb(asyncErr);
  ...
  PostgreSQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    ...
    aiNames.forEach(function(indexName) {
      ...
      m.properties[propName].index)) {
        sql.push('DROP INDEX ' + schemaName + '.' + self.escapeName(indexName));
      } else {
